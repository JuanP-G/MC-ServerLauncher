...
public class ServerProcessManager
{
    ...
    /// <summary>
    /// Locates the launch args file relative to the server folder (so it can be passed as
    /// <c>@&lt;relative-path&gt;</c> with the working directory set to the folder). Tries the exact
    /// version dir first, then any other under the same loader. Returns null if not found.
    /// </summary>
    /// <remarks>
    /// Forge and NeoForge keep this file in the same shape under different maven coordinates, so
    /// the root comes from <see cref="LoaderPaths"/> rather than being spelled out here — this used
    /// to hardcode Forge's, which is precisely how a second loader ends up installable but not
    /// startable.
    /// </remarks>
    private static string? ResolveForgeArgsFile(ServerConfig config)
    {
        var argName = LoaderPaths.ArgsFileName;
        var root = LoaderPaths.LibrariesRoot(config.FolderPath, config.Type);
        if (root is null || !Directory.Exists(root))
            return null;

        var exact = Path.Combine(root, config.ForgeArgs, argName);
